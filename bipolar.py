"""
====================================================
🔬 Verb Classification using ResNet18 (Classifier Chains Approach)
====================================================

📌 Dataset: JSON-formatted, base64-encoded laparoscopic surgery frames
📌 Task: Multi-label classification (10 verb classes)
📌 Filtering: Only frames containing 'bipolar' (instrument label = 1)
📌 Model: ResNet18 (pretrained), modified final FC layer
📌 Loss Function: BCEWithLogitsLoss (multi-label)
📌 Optimizer: Adam (lr = 0.001)
📌 Evaluation: Average Precision (AP) per class, Mean Average Precision (mAP)
📌 Classifier Chains: Subset of dataset filtered by presence of specific instrument ('bipolar') to model verb dependencies conditioned on instruments
...
class VerbDataset(Dataset):
    """
    Custom Dataset for loading base64-encoded surgical images and verb labels.
    Filters only those samples where the second instrument is 'bipolar'.
    """
    def __init__(self, json_file, transform=None):
        ...
        self.data = [d for d in data if d["instrument_labels"][1] == 1]  # Keep only if 2nd instrument is bipolar
        print(f"✅ Dataset samples (after filtering 'bipolar'): {len(self.data)}")
...
def compute_class_weights(dataset):
    ...
    class_weights /= np.max(class_weights)  # Normalize to max 1.0
...
class ResNetVerbClassifier(nn.Module):
    """
    Fine-tuned ResNet18 model for multi-label verb classification.
    """
    def __init__(self, num_classes=10):
        super(ResNetVerbClassifier, self).__init__()
        self.model = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
        self.model.fc = nn.Linear(self.model.fc.in_features, num_classes)  # Output raw logits
...
def evaluate_model(model, val_loader):
    """
    Evaluate using AP per class and restricted mAP on selected classes.
    """
    model.eval()
    all_targets = []
    all_preds = []
    ...
    with torch.no_grad():
        for images, labels in val_loader:
            ...
            outputs = torch.sigmoid(model(images))  # Probabilities
    ...
    if np.sum(all_targets) == 0:
        print("🚨 WARNING: No positive labels in validation set!")
    ...
    print(f"\n🔥 Restricted mAP (Class 0, 1, 2, 3, 9): {restricted_mAP:.4f}")
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=10):
    ...
    print("\n✅ Training complete. Best model weights saved as best_resnet_verb.pth")
...
if __name__ == "__main__":
    ...
    compute_class_weights(train_dataset)  # Optional: Use later in loss
    compute_class_distribution = lambda d, n: print(f"✅ Class distribution in {n}: {np.sum([l.numpy() for _, l in d], axis=0)}")
    ...
    criterion = nn.BCEWithLogitsLoss()  # Optionally add `pos_weight=class_weights`
