"""
============================================================
🔬 Verb Classification using ResNet18 (Classifier Chains: Hook)
============================================================

📌 Instrument Filtered: 'hook' (index 2 in instrument_labels)
🎯 Task: Multi-label verb classification (10 classes)
🧠 Model: ResNet18 (pretrained on ImageNet)
⚙️ Loss Function: BCEWithLogitsLoss (multi-label)
🚀 Optimizer: Adam (lr = 0.001)
📊 Evaluation: AP per class + restricted mAP (Classes 0, 1, 2, 3, 9)

💡 Classifier Chains: This experiment models verb classification *conditioned on the presence of 'hook'*, enabling targeted learning of co-occurring verb patterns.
...
class VerbDataset(Dataset):
    """
    Custom Dataset for surgical verb classification.
    Only includes samples where 'hook' (instrument index 2) is active.
    """
    def __init__(self, json_file, transform=None):
        ...
        self.data = [d for d in data if d["instrument_labels"][2] == 1]
        print(f"✅ Dataset samples (after filtering 'hook'): {len(self.data)}")
...
def compute_class_distribution(dataset, name="Dataset"):
    label_counts = np.zeros(10)
    for _, labels in dataset:
        label_counts += labels.numpy()
    print(f"📊 Class distribution in {name}: {label_counts}")

compute_class_distribution(val_dataset, "Validation")
...
def evaluate_model(model, val_loader):
    ...
    if np.sum(all_targets) == 0:
        print("🚨 WARNING: No positive labels in validation!")

    try:
        AP_per_class = [average_precision_score(all_targets[:, i], all_preds[:, i]) for i in range(10)]
    except ValueError:
        print("🚨 Some classes have no positive samples!")
        AP_per_class = [0.0] * 10
    ...
    print(f"\n🔥 Restricted mAP (Classes 0, 1, 2, 3, 9): {restricted_mAP:.4f}")
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=50):
    ...
    print("\n✅ Training complete. Best model saved as 'best_resnet_verb.pth'")
...
if __name__ == "__main__":
    ...
    print("✅ Training complete for instrument filter: 'hook'")
