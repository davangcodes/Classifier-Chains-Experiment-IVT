"""
============================================================
🔬 Verb Classification using ResNet18 (Classifier Chains: Irrigator)
============================================================

📌 Instrument Filtered: 'irrigator' (index 5 in instrument_labels)
🎯 Task: Multi-label verb classification (10 classes)
🧠 Model: ResNet18 (pretrained on ImageNet)
⚙️ Loss Function: BCEWithLogitsLoss (multi-label)
🚀 Optimizer: Adam (lr = 0.001)
📊 Evaluation: AP per class + restricted mAP (Classes 0, 1, 6, 7, 9)

💡 Classifier Chains: This experiment models verb classification *conditioned on the presence of 'irrigator'*, enabling targeted modeling of co-occurring actions.
...
class VerbDataset(Dataset):
    ...
    Filters frames where 'irrigator' (instrument index 5) is active.
    """
    def __init__(self, json_file, transform=None):
        ...
        self.data = [d for d in data if d["instrument_labels"][5] == 1]
        print(f"✅ Dataset samples (after filtering 'irrigator'): {len(self.data)}")
...
def compute_class_weights(dataset):
    ...
    if np.sum(label_counts) == 0:
        print("🚨 WARNING: No positive labels found in dataset!")
...
def evaluate_model(model, val_loader):
    ...
    if np.sum(all_targets) == 0:
        print("🚨 WARNING: No positive labels found in validation labels!")
    ...
    print(f"\n🔥 Restricted mAP (Classes 0, 1, 6, 7, 9): {restricted_mAP:.4f}")
...
if __name__ == "__main__":
    ...
    print("✅ Training complete for instrument filter: 'irrigator'")
