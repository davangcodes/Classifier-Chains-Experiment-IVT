"""
============================================================
🔬 Verb Classification using ResNet18 (Classifier Chains: Grasper)
============================================================

📌 Instrument Filtered: 'grasper' (index 0 in instrument_labels)
🎯 Task: Multi-label verb classification (10 classes)
🧠 Model: ResNet18 (pretrained on ImageNet)
⚙️ Loss Function: BCEWithLogitsLoss (multi-label)
🚀 Optimizer: Adam (lr = 0.001)
📊 Evaluation: AP per class + mAP across all 10 verb classes

💡 Classifier Chains: This experiment models verb classification *conditioned on the presence of 'grasper'*, enabling targeted learning.
...
class VerbDataset(Dataset):
    ...
    Only includes samples where instrument 'grasper' (index 0) is active.
    """
    def __init__(self, json_file, transform=None):
        ...
        # Filter based on 'grasper' (instrument_labels[0] == 1)
        self.data = [d for d in data if d["instrument_labels"][0] == 1]

        print(f"✅ Dataset samples (after filtering 'grasper'): {len(self.data)}")
...
def evaluate_model(model, val_loader):
    ...
    selected_APs = [ap for ap in AP_per_class if not np.isnan(ap)]
...
if __name__ == "__main__":
    ...
    print("✅ Finished training with 'grasper'-conditioned classifier chain.")
